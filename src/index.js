...
function noteKvLimitHit() { kvDisabledUntil = Date.now() + KV_BACKOFF_MS; }
function isKvEnabled() { return Date.now() >= kvDisabledUntil; }
...
async function raceProbe(poolName, healthyPool, timeoutMs = 1500) {
  try {
    ...
    const probes = healthyPool.map(async (b) => {
      const u = new URL(b.url); u.pathname = path;
...
async function readBodyOnce(req) {
  ...
  try { return await req.arrayBuffer(); } catch { return null; }
}
...
async function kvPutSafe(env, key, value, opts) {
  ...
  try {
    ...
    if (String(e?.message || e).toLowerCase().includes("limit")) noteKvLimitHit();
  }
}
...
async function proxyOnce(req, env, poolName, target, bodyBuf, measureRtt = true) {
  try {
    ...
    const TIMEOUT_MS = /^\/api\//.test(p) ? 25000 : 10000;
    const ac = new AbortController();
    const timer = setTimeout(() => ac.abort("upstream timeout"), TIMEOUT_MS);
    let resp = null;
    ...
    try {
      ...
      clearTimeout(timer);
      if (measureRtt && t0 != null && resp) {
        const dt = Date.now() - t0;
        if (Math.random() < RTT_SAMPLE && isKvEnabled() && canWriteRtt(rttKey)) {
          try {
            const rttMap = JSON.parse((await env.KV.get(rttKey)) || "{}");
  ...
  async fetch(req, env) {
    try {
      ...
      if (urlIn.pathname === "/__lb/health") {
        ...
        return new Response(JSON.stringify({ host, colo, kvEnabled, kvDisabledUntil, svc, adm, rttSvc, rttAdm }), { status: 200, headers: { "content-type": "application/json" } });
      }
      ...
      if (!target) target = healthyPool[0] || backends[0];
      ...
      if (!resp || resp.status >= 502 || resp.status === 0) {
        ...
        if (fallback) {
          const resp2 = await proxyOnce(req, env, poolName, fallback, bodyBuf, true);
          if (resp2) { resp = resp2; target = fallback; }
        }
      }
      ...
      for (const { name, list, path } of pools) {
        ...
        await Promise.all(list.map(async (b) => {
          try {
            ...
            u.pathname = path;
            const r = await fetch(u.toString(), { method: "GET", redirect: "follow", headers: { "User-Agent": "EdgeLB/1.0", "Accept": "*/*" } });
